...
CREATE TABLE `t_performance` (
    ...
    `perf_bonus_ratio` DECIMAL(5, 4)  NOT NULL DEFAULT 1.0000 COMMENT '绩效奖金系数（由评分计算，如0.8=80%绩效奖金）',
...
CREATE TABLE `t_tax_accumulate` (
    ...
    -- 本月数据
    `month_taxable_income`   DECIMAL(12, 2) NOT NULL DEFAULT 0.00 COMMENT '本月应纳税所得额（元）= 应发工资 - 社保个人扣 - 5000起征点 - 专项附加扣除',
    ...
    `month_special_deduct`   DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT '本月专项附加扣除（子女教育+继续教育+租房+房贷+赡养老人，元）',
    ...
    `accum_special_deduct`   DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT '年度累计专项附加扣除（元）',
...
CREATE TABLE `t_social_security_config` (
    ...
    `unemployment_emp_ratio` DECIMAL(5, 4) NOT NULL DEFAULT 0.0050 COMMENT '失业保险个人缴纳比例（0.5%）',
    `unemployment_comp_ratio` DECIMAL(5, 4) NOT NULL DEFAULT 0.0050 COMMENT '失业保险单位缴纳比例（0.5%）',
    `injury_comp_ratio`    DECIMAL(5, 4) NOT NULL DEFAULT 0.0050 COMMENT '工伤保险单位缴纳比例（个人不缴）',
    `fund_emp_ratio`       DECIMAL(5, 4) NOT NULL DEFAULT 0.0500 COMMENT '住房公积金个人缴纳比例（5%-12%）',
    `fund_comp_ratio`      DECIMAL(5, 4) NOT NULL DEFAULT 0.0500 COMMENT '住房公积金单位缴纳比例',
