# CHANGELOG
## 2026-04-13 V14.28（清理 backup_ui 历史备份页面目录）
### 仓库根目录 `backup_ui`
- [清理] 从 Git 仓库中删除根目录 `backup_ui` 目录，移除其中保留的 `admin/index.html`、`home.html`、`index.html`、`login.html` 四个历史备份页面文件。
- [原因] 该目录仅为早期页面备份，不属于当前系统正式入口或部署资源，继续保留会增加仓库噪音并造成“哪个页面才是正式版本”的歧义。
### DEV_PLAN.md
- [追踪] 新增并完成 `112` 号任务，记录本轮对 `backup_ui` 目录的仓库清理。
